...
uint64_t* tmp0 = make_array<uint64_t>( (int32_t)784);
/* Variable to read the clear value corresponding to the input variable tmp0 at (9027,1-9027,35) */
uint64_t __tmp_in_tmp0;
for (uint64_t i0 =  (uint64_t)0; i0 <  (int32_t)784; i0++){
if ((party == CLIENT)) {
cin >> __tmp_in_tmp0;
}
Arr1DIdxRowM(tmp0, (int32_t)784,i0) = (party == CLIENT) ? __tmp_in_tmp0 : 0;
}
StartComputation();

uint64_t* tmp1 = make_array<uint64_t>( (int32_t)784);
Relu1( (int32_t)784, tmp0, tmp1,  (int32_t)0, 0);
ClearMemSecret1( (int32_t)784, tmp0);

uint64_t* tmp3 = make_array<uint64_t>( (int32_t)784);
CreateIdentity11( (int32_t)784, tmp1, tmp3);
ClearMemSecret1( (int32_t)784, tmp1);
EndComputation();
for (uint64_t i0 =  (uint64_t)0; i0 <  (int32_t)784; i0++){
cout << (funcReconstruct2PCCons(Arr1DIdxRowM(tmp3, (int32_t)784,i0), 2)) << endl;
}
}
